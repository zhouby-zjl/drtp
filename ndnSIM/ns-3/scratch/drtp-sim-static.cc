...
vector<LltcLink*>* selectRandomNLinks(LltcGraph* g, list<comm_pair*>* commPairs, int n) {
...
int main(int argc, char** argv) {
	...
	double lossRate = rrg->computeResilientRouteEEFR(g, rr, config);
	vector<double>* lossRates = rrg->evaluateResilientRouteEEFR(lltcGraph, rr, config, 0, 0.99, 0.01,
			(double) config->numRetransRequests);

	cout << "lossRate: " << lossRate << endl;

	for (int k = 0; k < 100; ++k) {
		*logOut_lossRates << (double) k / 100.0 << "," << (*lossRates)[k] << endl;
	}

	logOut_lossRates->flush();
