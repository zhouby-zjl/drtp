...
vector<LltcLink*>* findDesignatedLinks(LltcGraph* g, string linkIDs_str) {
	vector<LltcLink*>* links_n = new vector<LltcLink*>();

	size_t n_str = linkIDs_str.size();
	char str[n_str];
	strncpy(str, linkIDs_str.c_str(), n_str);
	char *ID_str = strtok(str, ",");
	while (ID_str != NULL) {
		int link_id = atoi(ID_str);
		ID_str = strtok(NULL, ",");

		LltcLink* link = g->getLinkByID(link_id);
		links_n->push_back(link);
	}
...
int main(int argc, char** argv) {
	...
	switch (faultSim_Type) {
		...
		case FAULT_SIM_TYPE_ERR_FOR_DESIGNATED_LINKS: {
			vector<LltcLink*>* linksToMakeFault = findDesignatedLinks(g, LltcConfig::FAULT_SIM_TYPE_ERR_FOR_DESIGNATED_LINKS_IDS);
			simulateLinkErrors(LltcConfig::FAULT_SIM_TYPE_ERR_FOR_DESIGNATED_LINKS_START_TIME,
					LltcConfig::FAULT_SIM_TYPE_ERR_FOR_DESIGNATED_LINKS_FLAP_PERIOD,
					LltcConfig::FAULT_SIM_TYPE_ERR_FOR_DESIGNATED_LINKS_FLAP_TIMES,
					linksToMakeFault, &nodes,
