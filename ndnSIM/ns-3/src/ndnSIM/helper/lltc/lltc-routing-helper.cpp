...
LltcRoutingHelper::LltcRoutingHelper(LltcGraph* lltcGraph, LltcConfiguration* config, int routingType) {
	if (routingType == ROUTING_TYPE_DRTP) {
		rrg = new ResilientRouteGenerationForDRTP();
	} else if (routingType == ROUTING_TYPE_REROUTE) {
		rrg = new ResilientRouteGenerationForReroute();
	} else if (routingType == ROUTING_TYPE_DRTP_DIJKSTRA) {
		rrg = new ResilientRouteGenerationForDrtpSimple();
	} else if (routingType == ROUTING_TYPE_STATIC) {
		rrg = new ResilientRouteGenerationForStatic();
	}
...
void LltcRoutingHelper::computeRoutesVectors() {
	if (routingType != ROUTING_TYPE_DRTP && routingType != ROUTING_TYPE_DRTP_DIJKSTRA
			&& routingType != ROUTING_TYPE_REROUTE)  return;
...
void LltcRoutingHelper::computeRoutesAndAddCommPairsForStaticRoutes(NodeContainer* nodes) {
	if (routingType != ROUTING_TYPE_STATIC) return;

	rrvMap_static = rrg->genResilientRoutesVectors(lltcGraph, -1, config);
	...
	unordered_map<int, ResilientRoutes*>* all_rrs = rrvMap_static->all_rrs;
	for (unordered_map<int, ResilientRoutes*>::iterator iter = all_rrs->begin(); iter != all_rrs->end(); ++iter) {
		ResilientRoutes* rr = iter->second;
		int producerNodeId = iter->first / 0xffff;
		int consumerNodeId = iter->first %  0xffff;
		Ptr<Node> producerNode = nodes->Get(producerNodeId);
		Ptr<Node> consumerNode = nodes->Get(consumerNodeId);
		Ptr<LltcRouter> producerRouter = producerNode->GetObject<LltcRouter>();
		Ptr<LltcRouter> consumerRouter = consumerNode->GetObject<LltcRouter>();

		ss.str("");
		ss << "/lltc/pmu" << producerNodeId;
		const char* producerName = ss.str().c_str();

		this->AddOrigin(producerName, producerNode);
		this->addCommPair(producerNode, consumerNode, rr);
...
vector<pair<int, int>>* LltcRoutingHelper::getCommPairIDsForStaticRoutes() {
	vector<pair<int, int>>* commPairs = new vector<pair<int, int>>();
	unordered_map<int, ResilientRoutes*>* all_rrs = rrvMap_static->all_rrs;
	for (unordered_map<int, ResilientRoutes*>::iterator iter = all_rrs->begin(); iter != all_rrs->end(); ++iter) {
			ResilientRoutes* rr = iter->second;
			int producerNodeId = iter->first / 0xffff;
			int consumerNodeId = iter->first %  0xffff;
			pair<int, int> p;
			p.first = producerNodeId;
			p.second = consumerNodeId;
			commPairs->push_back(p);
	}
	return commPairs;
}
...
void LltcRoutingHelper::addCommPair(Ptr<Node> producerNode, Ptr<Node> consumerNode, ResilientRoutes* rr) {
	comm_pair* commPair = new comm_pair();
	commPair->nodes.first = producerNode;
	commPair->nodes.second = consumerNode;
	commPair->rr = rr;
