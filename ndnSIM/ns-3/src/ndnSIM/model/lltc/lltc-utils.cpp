...
void LltcLog::openLogs(int nRouters) {
	...
	ss.str("");
	ss << *logDirPath << "log-rr-relia-evaluation";
	logRrReliaEvaluation = new ofstream;
	logRrReliaEvaluation->open(ss.str(), ios::trunc);
...
ofstream* LltcLog::getLogRrReliaEvaluation() {
...
size_t LltcRoutesDumper::getNDataIdsInRequestM(const Data& data) {
	list<LltcDataID> lostDataIds;
	LltcMessagesHelper::extractRequestM(data, &lostDataIds);
	return lostDataIds.size();
}

size_t LltcRoutesDumper::getNDataIdsInReport(const Data& data) {
	list<LltcDataID> lostDataIds;
	LltcMessagesHelper::extractReport(data, &lostDataIds);
	return lostDataIds.size();
}
...
string LltcRoutesDumper::dumpMsg(bool isPIT, bool isRSG, uint32_t rsgId, uint32_t pathId, const FaceEndpoint& port, const Data& data) {
	...
	if (dataNameStr.find("RetranM") != dataNameStr.npos) {
		size_t n = getNDataIdsInRequestM(data);
		ss << "," << n;
	} else if (dataNameStr.find("Report") != dataNameStr.npos) {
		size_t n = getNDataIdsInReport(data);
		ss << "," << n;
	}
