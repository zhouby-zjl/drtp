...
double FHKeyFuncDelay::PRECISION = 1;

FHKeyFuncDelay::FHKeyFuncDelay(int* delays) {
	this->delays = delays;
}

long FHKeyFuncDelay::getKey(FHObjectPtr nodeObject) {
	int nodeId = *((int*) (nodeObject));
	int delay = delays[nodeId];
	long key = -(long) (delay * PRECISION);
	return key;
}

void FHKeyFuncDelay::setKey(FHObjectPtr nodeObject, long newKey) {
	int nodeId = *((int*) (nodeObject));
	double delay = -(double)(newKey) / PRECISION;
	delays[nodeId] = delay;
}
