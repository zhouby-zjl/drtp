...
string LltcConfig::RR_STATIC_FILE = "";
...
bool LltcConfig::LLTC_ENABLE_FAILOVER  = false;
bool LltcConfig::LLTC_ENABLE_FAILOVER_ACTIVE_CHECK = false;
...
bool LltcConfig::loadConfigFile(string filePath) {
	...
	while (getline(f, line)) {
		if (regex_match(line, m, setPattern)) {
			...
			} else if (name.compare("RR_STATIC_FILE") == 0) {
				RR_STATIC_FILE = value;
