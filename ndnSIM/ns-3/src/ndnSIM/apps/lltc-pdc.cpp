...
void LltcPDCApp::StartApplication() {
	...
	Simulator::Schedule(Seconds(15.0), &LltcPDCApp::SendUpSideControlMessage, this);
...
void LltcPDCApp::SendUpSideControlMessage() {
	for (pair<string, PmuState*> pmuState : pmuStateMap) {
		shared_ptr<Data> data = LltcMessagesHelper::constructUpSideControl(pmuState.first, nodeId, pmuState.second->nodeID, std::make_shared<::ndn::Buffer>(1024));
