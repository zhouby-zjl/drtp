...
unordered_map<int, unordered_map<string, int>*> R2tStrategy::performance_res;
...
R2tStrategy::R2tStrategy(Forwarder& forwarder, const Name& name) : Strategy(forwarder) {
	...
	R2tStrategy::performance_res[routes->nodeID] = new unordered_map<string, int>();
}
...
void R2tStrategy::afterReceiveInterest(const FaceEndpoint& ingress, const Interest& interest,
	...
	if (R2tStrategy::performance_res.find(routes->nodeID) != R2tStrategy::performance_res.end()) {
		unordered_map<string, int>* res = R2tStrategy::performance_res[routes->nodeID];
