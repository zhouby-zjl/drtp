...
unordered_map<int, unordered_map<string, int>*> SdatpStrategy::performance_res;
...
SdatpStrategy::SdatpStrategy(Forwarder& forwarder, const Name& name) : Strategy(forwarder) {
	...
	recentDataID = -1;
	SdatpStrategy::performance_res[routes->nodeID] = new unordered_map<string, int>();
}
...
void SdatpStrategy::afterReceiveInterest(const FaceEndpoint& ingress, const Interest& interest,
	...
	if (SdatpStrategy::performance_res.find(routes->nodeID) != SdatpStrategy::performance_res.end()) {
		unordered_map<string, int>* res = SdatpStrategy::performance_res[routes->nodeID];
