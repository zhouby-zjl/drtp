...
void LltcStrategy::afterReceiveInterest(const FaceEndpoint& ingress, const Interest& interest,
	...
	createOrGetCapsuleForwardingStates(name.toUri(name::UriFormat::DEFAULT));
}
...
void LltcStrategy::afterReceiveData(const shared_ptr<pit::Entry>& pitEntry,
	...
	CapsuleForwardingStates* ds = createOrGetCapsuleForwardingStates(pitPrefixStr);
	...
	} else if (operationStr.compare(LLTC_MSG_OP_REPORT) == 0) {
		*outLog_in_msgs << LltcRoutesDumper::dumpMsg(true, true, rsgId, pathId, ingress, data) << endl;
		this->onReceiveReport(pitEntry, ingress, data, ds, rsgId, pathId);
		return;
	}

	int direction = -1;
	if (operationStr.compare(LLTC_MSG_OP_RETRANS_IN_SINGLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_RETRANS_IN_MULTIPLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_CAPSULE_ON_RSP) == 0 ||
			operationStr.compare(LLTC_MSG_OP_DOWN_SIDE_CONTROL) == 0) {
		direction = 0;
	} else if (operationStr.compare(LLTC_MSG_OP_REQUEST_IN_SINGLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_REQUEST_IN_MULTIPLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_UP_SIDE_CONTROL) == 0) {
		direction = 1;
	} else {
		return;
	}

	bool needToForward = true;
	if (operationStr.compare(LLTC_MSG_OP_RETRANS_IN_SINGLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_RETRANS_IN_MULTIPLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_REQUEST_IN_SINGLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_REQUEST_IN_MULTIPLE) == 0) {
		if (pathId == this->getForwardingState(rsgId)->primaryPathId) {
			needToForward = false;
		}
	}
	...
	*outLog_in_msgs << LltcRoutesDumper::dumpMsg(true, true, rsgId, pathId, ingress, data) << endl;
	...
	if (operationStr.compare(LLTC_MSG_OP_REQUEST_IN_SINGLE) == 0) {
		this->onReceiveRequestS(pitEntry, ingress, data, ds, rsgId, pathId);

	} else if (operationStr.compare(LLTC_MSG_OP_REQUEST_IN_MULTIPLE) == 0) {
		this->onReceiveRequestM(pitEntry, ingress, data, ds, rsgId, pathId);

	} else if (operationStr.compare(LLTC_MSG_OP_RETRANS_IN_SINGLE) == 0) {
		this->onReceiveRetranS(pitEntry, ingress, data, ds, rsgId, pathId);

	} else if (operationStr.compare(LLTC_MSG_OP_RETRANS_IN_MULTIPLE) == 0) {
		this->onReceiveRetranM(pitEntry, ingress, data, ds, rsgId, pathId);

	} else if (operationStr.compare(LLTC_MSG_OP_CAPSULE_ON_RSP) == 0) {
		this->onReceiveCapsuleOnRsp(pitEntry, ingress, data, ds, rsgId, pathId);

	} else if (operationStr.compare(LLTC_MSG_OP_DOWN_SIDE_CONTROL) == 0) {
		this->onReceiveDownSideControl(pitEntry, ingress, data, ds, rsgId, pathId);
...
void LltcStrategy::afterReceiveNonPitData(const FaceEndpoint& ingress, const Data& data) {
	...
	if (operationStr.compare(LLTC_MSG_OP_REQUEST_IN_SINGLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_REQUEST_IN_MULTIPLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_UP_SIDE_CONTROL) == 0) {
		LltcFIBEntry* feUp = this->lookupForwardingEntry(rsgId, pathId, 1);
		if (feUp != NULL) {
			this->sendLltcNonPitData(data, FaceEndpoint(*feUp->face_nextHop, 0), rsgId, pathId);
		}

	} else if (operationStr.compare(LLTC_MSG_OP_RETRANS_IN_SINGLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_RETRANS_IN_MULTIPLE) == 0 ||
			operationStr.compare(LLTC_MSG_OP_CAPSULE_ON_RSP) == 0 ||
			operationStr.compare(LLTC_MSG_OP_REPORT) == 0 ||
			operationStr.compare(LLTC_MSG_OP_DOWN_SIDE_CONTROL) == 0) {
